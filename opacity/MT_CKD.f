C--YF--ADDED----------------------
C===========================================
C This code is based on the MT_CKD continuum
C software package developed by Atmospheric 
C and Environmental Research, obtained from 
C http://rtweb.aer.com/continuum_code.html
C Y.F. modified the original source code a 
C little in order to call it from python. 
C   TIME STAMP: Feb 12, 2015
C===========================================
...
C  --------------------------------------------------------------------------
C
      SUBROUTINE get_cntnm(p_in, t_in, vi, csh2or, cfh2or)

Cf2py intent(in) :: p_in, t_in
Cf2py intent(out) :: vi, csh2or, cfh2or
C
C
...
C                                                                         F00100
      ...
      REAL*8 SECANT, XALTZ, p_in, t_in

      REAL*8 vi(5050), csh2or(5050), cfh2or(5050)
c
      character*8 holn2
C                                                                         F00120
...
C      OPEN (ipu,FILE='WATER.COEF')
C---------------------------------
c
C--YF--DELETED--------------------
C---------------------------------                   
C      print *
C     print *, '  This version is limited to 5000 values  '
C---------------------------------                   
...
CC
C  970 FORMAT (/,29x, 'P(MB)',7X,'T(K)', //,23x,0P,F12.3,F9.2)
C  975 FORMAT (/, 9X,'MOLECULAR AMOUNTS (MOL/CM**2) BY LAYER ',//,
C     *         8(1X,A6,3X))
C  980 FORMAT (/,1P,8E10.3,//)
C---------------------------------
...
C---------------------------------
C      DO 100 I=1,NPTABS
C      VI=V1ABS+FLOAT(I-1)*DVABS
C 100  WRITE (ipr, 910) VI, ABSRB(I) 
C 910  FORMAT(F10.3,1P,E12.3)
C---------------------------------
...
c
      ...
      do 200 i=1,npth
      vi(i)=v1h+float(i-1)*dvh
      if (vi(i).ge.v1abs .and. vi(i).le.v2abs) then
         radfld=radfn(vi(i),xkt)
         csh2or(i) = csh2o(i) * radfld
         cfh2or(i) = cfh2o(i) * radfld

C--YF--DELETED--------------------
C---------------------------------
C         write (ipu,930) vi, csh2o(i), cfh2o(i), csh2or, cfh2or
C  930    format(f10.2, 1p, 2e15.4,10x, 1p, 2e15.4)
C---------------------------------
...
C--YF--DELETED--------------------
C---------------------------------
C      common /CDERIV/ icflg,iuf,v1absc,v2absc,dvabsc,nptabsc,delT_pert,
C     &    dqh2oC(ipts),dTh2oC(ipts),dUh2o
C---------------------------------
...
C--YF--DELETED--------------------
C---------------------------------
C      WRITE (IPR,910) ((CINFO1(I,J),I=1,2),J=1,11)
C      WRITE (IPR,910) ((CINFO2(I,J),I=1,2),J=1,11)
C      WRITE (IPR,918) ((CHEADING(I,J),I=1,3),J=1,2)
C      WRITE (IPR,915) (cnam3(j),(CINFO3(I,J),I=1,2),J=1,9)
C      WRITE (IPR,915) (cnam4(j),(CINFO4(I,J),I=1,2),J=1,29)
C---------------------------------
...
C--YF--DELETED--------------------
C---------------------------------
C 910  FORMAT (18x,2A51)
C 915  FORMAT (a18,2A51)
C 918  FORMAT (3A40)
C---------------------------------
C                                                                         A10580
